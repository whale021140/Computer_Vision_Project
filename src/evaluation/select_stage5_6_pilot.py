"""Choose the Stage 5.6 final recipe using the locked development set only."""
...
def main() -> None:
    ...
    for order, variant in enumerate(VARIANTS):
        root = Path("outputs/stage5_6/cells") / (
            f"siglip2_10pct_seed0_{variant}"
        )
        calibration = json.loads(
            (root / "calibration_dev.json").read_text(encoding="utf-8")
        )
        selection = json.loads(
            (root / "selection_dev.json").read_text(encoding="utf-8")
        )
        rows.append(
            {
                "variant": variant,
    ...
    output_json = Path("outputs/stage5_6/pilot_selection.json")
    output_txt = Path("outputs/stage5_6/pilot_selection.txt")
    output_json.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    lines = [
        "Stage 5.6 Pilot Selection",
        "=========================",
